TASK: Compose a counterpoint voice ABOVE the given cantus firmus in fourth species counterpoint (syncopated half notes tied across barlines). The counterpoint must be on STAFF 1 (top staff) and the cantus firmus on STAFF 2 (bottom staff).

VOICE PLACEMENT:
- The cantus firmus is the LOWER voice. You MUST compose the counterpoint as the UPPER voice.
- Assign treble clef to the counterpoint voice.
...
ENCODING ACCURACY:
- Preserve the encoding conventions from the input file: namespace declarations, attribute ordering, indentation style.
